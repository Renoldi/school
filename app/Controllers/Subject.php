...
{
    protected $modelName = ModelsSubject::class;
    protected $format    = 'json';
    use ResponseTrait;
    /**
     * @OA\Get(
     *   path="/api/Subject/paging/{status}/{perpage}/{page}",
     *   summary="Subject",
     *   description="Subject",
     *   tags={"Subject"},
     *   @OA\Parameter(
    ...
    public function paging($status = "all", $perpage = 20, $page = 1)
    {
        $model = $this->model;
        if ($status == 1) {
            $model = $this->model->where(['Subject.statusId' => 1]);
        } elseif ($status == 0) {
            $model = $this->model->where(['Subject.statusId' => 0]);
        }

        $data = $model
        ->select('Subject.*, s.name statusName')
        ->join('status s', 's.id=Subject.statusId')
    ...
    public function index()
    {
        ...
        ->select('Subject.*, s.name statusName')
        ->join('status s', 's.id=Subject.statusId')
        ->where('Subject.statusId', 1)->findAll());
    }
    ...
    public function show($id = null)
    {
        ...
        ->select('Subject.*, s.name statusName')
        ->join('status s', 's.id=Subject.statusId')
        ->where('Subject.statusId', 1)->find($id);
        if (!$record) {
    ...
    public function  fromFile()
    {
        ...
        $subjectEntity = new EntitiesSubject();
