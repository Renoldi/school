...
{
    protected $modelName = ModelsMajor::class;
    protected $format    = 'json';
    use ResponseTrait;
    /**
     * @OA\Get(
     *   path="/api/Major/paging/{status}/{perpage}/{page}",
     *   summary="Major",
     *   description="Major",
     *   tags={"Major"},
     *   @OA\Parameter(
     ...
     *      @OA\JsonContent(ref="#/components/schemas/Major")
     *   ),
     *   @OA\Response(
    ...
    public function paging($status = "all", $perpage = 20, $page = 1)
    {
        $model = $this->model;
        if ($status == 1) {
            $model = $this->model->where(['Major.statusId' => 1]);
        } elseif ($status == 0) {
            $model = $this->model->where(['Major.statusId' => 0]);
        }

        $data = $model
        ->select('Major.*, s.name statusName')
        ->join('status s', 's.id=Major.statusId')
     ...
     *     @OA\JsonContent(
     *      type="array",
     *       @OA\Items(ref="#/components/schemas/Major")
     *     ),
     *   ),
     *   @OA\Response(
     *     response=400, description="Bad Request"
    ...
    public function show($id = null)
    {
        ...
        ->select('Major.*, s.name statusName')
        ->join('status s', 's.id=Major.statusId')
        ->where('Major.statusId', 1)->find($id);
        if (!$record) {
     ...
     *      @OA\Schema(ref="#/components/schemas/Major"),
     *     )
     *   ),
     *   @OA\Response(
    ...
    public function  fromFile()
    {
        ...
        $subjectEntity = new EntitiesMajor();
