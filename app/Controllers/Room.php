...
{
    protected $modelName = ModelsRoom::class;
    protected $format    = 'json';
    use ResponseTrait;
    /**
     * @OA\Get(
     *   path="/api/Room/paging/{status}/{perpage}/{page}",
     *   summary="Room",
     *   description="Room",
     *   tags={"Room"},
     *   @OA\Parameter(
     ...
     public function paging($status = 1, $perpage = 20, $page = 1)
    {
        $model = $this->model;
        if ($status != 0) {
            $model = $this->model->where(['rooms.statusId' => $status]);
        } 
    ...
    public function  fromFile()
    {
        ...
        $subjectEntity = new EntitiesRoom();
