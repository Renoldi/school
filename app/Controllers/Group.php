...
{
    protected $modelName = ModelsGroup::class;
    protected $format    = 'json';
    use ResponseTrait;
    /**
     * @OA\Get(
     *   path="/api/Group/paging/{status}/{perpage}/{page}",
     *   summary="Group",
     *   description="Group",
     *   tags={"Group"},
     *   @OA\Parameter(
    ...
    public function paging($status = "all", $perpage = 20, $page = 1)
    {
        $model = $this->model;
        if ($status == 1) {
            $model = $this->model->where(['group.statusId' => 1]);
        } elseif ($status == 0) {
            $model = $this->model->where(['group.statusId' => 0]);
        }

        $data = $model
            ->select('group.*, s.name statusName')
            ->join('status s', 's.id=group.statusId')
    ...
    public function show($id = null)
    {
        $record = $this->model
            ->select('group.*, s.name statusName')
            ->join('status s', 's.id=group.statusId')
            ->where('group.statusId', 1)->find($id);
        if (!$record) {
    ...
    public function  fromFile()
    {
        ...
        $subjectEntity = new EntitiesGroup();
