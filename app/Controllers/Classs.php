...
class Classs extends ResourceController
{
    ...
    /**
     * @OA\Get(
     *   path="/api/Classs/paging/{status}/{perpage}/{page}",
     *   summary="Classs",
     *   description="Classs",
     *   tags={"Classs"},
     *   @OA\Parameter(
    ...
    public function paging($status = "all", $perpage = 20, $page = 1)
    {
        $model = $this->model;
        if ($status == 1) {
            $model = $this->model->where(['classs.statusId' => 1]);
        } elseif ($status == 0) {
            $model = $this->model->where(['classs.statusId' => 0]);
        }

        $data = $model
            ->select('classs.*, s.name statusName')
            ->join('status s', 's.id=classs.statusId')
    ...
    public function show($id = null)
    {
        $record = $this->model
            ->select('classs.*, s.name statusName')
            ->join('status s', 's.id=classs.statusId')
            ->where('classs.statusId', 1)->find($id);
        if (!$record) {
