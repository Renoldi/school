...
{
    protected $modelName = ModelsSemester::class;
    protected $format    = 'json';
    use ResponseTrait;
    /**
     * @OA\Get(
     *   path="/api/Semester/paging/{status}/{perpage}/{page}",
     *   summary="Semester",
     ...
     *      @OA\JsonContent(ref="#/components/schemas/Semester")
     *   ),
     *   @OA\Response(
    ...
    public function paging($status = "all", $perpage = 20, $page = 1)
    {
        $model = $this->model;
        if ($status == 1) {
            $model = $this->model->where(['Semester.statusId' => 1]);
        } elseif ($status == 0) {
            $model = $this->model->where(['Semester.statusId' => 0]);
        }

        $data = $model
            ->select('Semester.*, s.name statusName')
            ->join('status s', 's.id=Semester.statusId')
    ...
    public function show($id = null)
    {
        $record = $this->model
            ->select('Semester.*, s.name statusName')
            ->join('status s', 's.id=Semester.statusId')
            ->where('Semester.statusId', 1)->find($id);
        if (!$record) {
     ...
     *      @OA\Schema(ref="#/components/schemas/Semester"),
     *     )
     *   ),
     *   @OA\Response(
    ...
    public function  fromFile()
    {
        ...
        $subjectEntity = new EntitiesSemester();
