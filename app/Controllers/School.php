...
{
    protected $modelName = ModelsSchool::class;
    protected $format    = 'json';
    use ResponseTrait;
    /**
     * @OA\Get(
     *   path="/api/School/paging/{status}/{perpage}/{page}",
     *   summary="School",
     *   description="School",
     *   tags={"School"},
     *   @OA\Parameter(
    ...
    public function paging($status = "all", $perpage = 20, $page = 1)
    {
        $model = $this->model;
        if ($status == 1) {
            $model = $this->model->where(['School.statusId' => 1]);
        } elseif ($status == 0) {
            $model = $this->model->where(['School.statusId' => 0]);
        }

        $data = $model
        ->select('School.*, s.name statusName')
        ->join('status s', 's.id=School.statusId')
    ...
    public function show($id = null)
    {
        ...
        ->select('School.*, s.name statusName')
        ->join('status s', 's.id=School.statusId')
        ->where('School.statusId', 1)->find($id);
        if (!$record) {
            return $this->failNotFound(sprintf(
                'not found',
                $id
    ...
    public function  fromFile()
    {
        ...
        $subjectEntity = new EntitiesSchool();
